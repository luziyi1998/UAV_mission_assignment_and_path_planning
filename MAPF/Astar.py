...
def astar_with_conflict_avoid(transit_network, transit_constarin, transit_edges, depot_id, package_id, Max_flight):
    """
    ...
    for edge in transit_edges:
        transit_wait[(edge[0], edge[1])] = wait_times * np.random.rand() * transit_network.edges[edge]['time']
        ...
        for neighbor, w in transit_network[cur.node_id].items():
            if cur.flight_cost + transit_network.edges[cur.node_id, neighbor]['weight'] < Max_flight:
                ...
                if transit_network.edges[cur.node_id, neighbor]['type'] == 'transit':
                    ...
                    extra_wait = conflict_exit(transit_constarin[cur.node_id], cur.time_cost, transit_wait[(cur.node_id, neighbor)])
                ...
                if neighbor == package_id:
                    add_node = A(neighbor, cur.time_cost + transit_network.edges[cur.node_id, neighbor]['time'] + transit_wait[(cur.node_id, neighbor)], cur.flight_cost + transit_network.edges[cur.node_id, neighbor]['weight'], path)
                    pq.push(add_node, add_node.time_cost)
                else:
                    add_node = A(neighbor, cur.time_cost + transit_network.edges[cur.node_id, neighbor]['time'] + transit_wait[(cur.node_id, neighbor)], cur.flight_cost + transit_network.edges[cur.node_id, neighbor]['weight'], path)
                    pq.push(add_node, add_node.time_cost + transit_network.edges[neighbor, package_id]['time'])
